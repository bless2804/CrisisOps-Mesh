...
/** --- Constants & Labels --- */
const CORE_TABS: Agency[] = ["law", "fire", "ems", "hospitals", "utilities", "transport", "ngos"]; // NGOs always visible
...
// Gate publishing so the UI can be read-only until backend is ready
...
function AppInner() {
  ...
  const tabs = CORE_TABS; // NGOs always included

  // Commands (gated)
  async function handleAction(a: "ack" | "assign" | "escalate" | "resolve", inc: Incident) {
    if (!COMMANDS_ENABLED) {
      push("Commands disabled (frontend-only demo)");
      return;
    }
    const s = sessionRef.current as any;
    if (!s) {
      push("Not connected to broker");
      return;
    }
    const id = inc.id ?? "";
    try {
  ...
  return (
    // Force dark mode by adding `dark` on this container
    <main className="dark min-h-screen bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      {/* Header (raised z-index so it always stays above the map) */}
      <header className="sticky top-0 z-[3000] border-b bg-white/80 backdrop-blur dark:bg-slate-950/80 dark:border-slate-800">
      ...
      <div className="max-w-6xl mx-auto p-4 grid gap-4">
        ...
        {/* Agency Queues — moved above the event list */}
          ...
          {visible.map((e, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-sm text-gray-200">{e.headline || e.summary || "—"}</div>
                {e.location && (
